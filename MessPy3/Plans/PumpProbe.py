from atom.api import (List, Typed, Bool, Float, Value, Int, Enum, Atom,
                      ForwardTyped, Instance, Event, List)
from atom.atom import set_default
from MessPy3.Plans.common import SampleInfo, SetupInfo, Meta, Plan
from Instruments.interfaces import Cam, TuneableCam, DelayLine, Shutter, RotationStage
import tables as tb
import zarr
...
class PumpProbeCamData:
    ...
    def __init__(self):
        ...
        self.data.attrs['names'] = dict(signals=cam.sig_lines,
                                        stds=cam.std_lines,
                                        lines=cam.lines)
...
class PumpProbe(Plan):
    required_instruments = [DelayLine, Cam]
    optional_instruments = [RotationStage, Shutter]

    cam_list: List = List(Instance(Cam))
    cam_results = Value()
    delay_line: DelayLine = Typed(DelayLine)
    settings: PumpProbeSettings = Typed(PumpProbeSettings)
    rot_stage: T.Optional[RotationStage] = Typed(RotationStage)
    shutter: T.Optional[Shutter] = Typed(Shutter)
